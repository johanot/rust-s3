...
impl Bucket {
        ...
        loop {
            ...
            if chunk.len() < CHUNK_SIZE {
                if part_number == 0 {
                    ...
                    part_number += 1;
                    let part = self.put_multipart_chunk(chunk, &path, part_number, upload_id).await?;
                    ...
                    let inner_data = etags
                        ...
                        .collect::<Vec<Part>>();
                    code = self.complete_multipart_upload(&path, upload_id, inner_data).await?;
    ...
    #[maybe_async::async_impl]
    pub async fn initiate_multipart_upload(&self, s3_path: &str) -> Result<InitiateMultipartUploadResponse> {
        ...
        let msg: InitiateMultipartUploadResponse = serde_xml::from_str(std::str::from_utf8(data.as_slice())?)?;
        Ok(msg)
    }

    #[maybe_async::sync_impl]
    pub fn initiate_multipart_upload(&self, s3_path: &str) -> Result<InitiateMultipartUploadResponse> {
        let command = Command::InitiateMultipartUpload;
        let request = RequestImpl::new(self, &s3_path, command);
    ...
    #[maybe_async::async_impl]
    pub async fn put_multipart_stream<R: AsyncRead + Unpin>(&self, reader: &mut R, path: &str, part_number: u32, upload_id: &str) -> Result<Part> {
        let chunk = crate::utils::read_chunk(reader).await?;
        self.put_multipart_chunk(chunk, path, part_number, upload_id).await
    }
    ...
    #[maybe_async::async_impl]
    pub async fn put_multipart_chunk(&self, chunk: Vec<u8>, path: &str, part_number: u32, upload_id: &str) -> Result<Part> {
    ...
    #[maybe_async::sync_impl]
    pub fn put_multipart_chunk(&self, chunk: Vec<u8>, path: &str, part_number: u32, upload_id: &str) -> Result<Part> {
        let command = Command::PutObject {
            content: &chunk,
            content_type: "application/octet-stream",
            multipart: Some(Multipart::new(part_number, upload_id)),
    ...
    #[maybe_async::async_impl]
    pub async fn complete_multipart_upload(&self, path: &str, upload_id: &str, parts: Vec<Part>) -> Result<u16> {
    ...
    #[maybe_async::sync_impl]
    pub fn complete_multipart_upload(&self, path: &str, upload_id: &str, parts: Vec<Part>) -> Result<u16> {
        let data = CompleteMultipartUploadData { parts };
        let complete = Command::CompleteMultipartUpload {
            upload_id,
            data,
